...
class SaleInvoiceWithPreviousPayments extends TransformerAbstract
{
    use AddressTrait, PaymentHistoryTrait;
    /**
    ...
    /* private function paymentHistory(SaleInvoices $si): array
    {
        if ($si->receipts) {
            return $si->receipts->map(function ($receipt) {
                return [
                    ...
                    'saldo' => $receipt->saldo,
                    'percentage_payment' => $receipt->pivot->percentage_payment,
                    'import_payment' => $receipt->pivot->import_payment,
                    'percentage_paid_history' => $receipt->pivot->percentage_paid_history,
                    'import_paid_history' => $receipt->pivot->import_paid_history,
                    'created_at' => Carbon::parse($receipt->pivot->created_at)->format('d-m-Y'),
                ];
            })->toArray();
        }
    ...
    public function transform(SaleInvoices $si)
    {
        ...
        return [
            ...
            'paymentHistory' => $this->paymentHistory($si),
        ];
    }
}
